...
IndProperties$trait[780]=0.59146901
head(IndProperties)
# Some analysis
sortBytrait=IndProperties[order(IndProperties$trait),]
sortByTR=IndProperties[order(IndProperties$tolerance_risk),]
par(mfrow=c(2,3))
plot(sortBytrait$trait,sortBytrait$Simult_colaborations)
plot(sortByTR$tolerance_risk,sortByTR$Simult_colaborations,col=4)
scatter3D(sortByTR$trait,sortByTR$tolerance_risk,sortByTR$Simult_colaborations,phi=10,theta=45,
          type="p",pch=19,xlab="Trait",ylab="Tolerance Risk",zlab="Simultaneous colaborations")

plot(sortBytrait$trait,sortBytrait$Tot_colaborations)
plot(sortByTR$tolerance_risk,sortByTR$Tot_colaborations,col=4)
scatter3D(sortByTR$trait,sortByTR$tolerance_risk,sortByTR$Tot_colaborations,phi=10,theta=45,
          type="p",pch=19,xlab="Trait",ylab="Tolerance Risk",zlab="Total colaborations")
par(mfrow=c(1,1))
...
zzz=matrix(trend.table.ord$pubScore1,100,1000)
zzz2=matrix(trend.table.ord$pubScore3,1000,100,byrow = T)
image(1:100,IndProperties$trait[order(IndProperties$trait)],zzz,col=rainbow(max(trend.table.ord$pubScore1)))
image(IndProperties$trait[order(IndProperties$trait)],1:100,zzz2,col=rainbow(max(trend.table.ord$pubScore3)))
...
scatter3D(rep(1:100,each=1000),trend.table$Trait,trend.table$pubScore1,phi=45,theta=45,
          type="h",pch=19,xlab="Time",ylab="Trait",zlab="Publication Rate",cex=0.2)
...
for(i in 1:999)
{
  if(IndProperties$trait[order(IndProperties$trait)][i]==IndProperties$trait[order(IndProperties$trait)][i+1])
    {
      numb=c(i,j)
      break
    }  
}
which(IndProperties$trait==0.59146900)
